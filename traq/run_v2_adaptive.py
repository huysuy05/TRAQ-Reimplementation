# run_v2_adaptive.py
# TRAQ retriever-set + Adaptive Rejection CP — with adaptive NE sampling.
#
# Key difference from run_v2.py:
#   Instead of sampling a fixed M times and computing NE once, we sample in
#   mini-batches (e.g. 3 at a time) and check after each batch whether NE
#   has converged:
#
#     batch 1:  sample 3  → compute NE_3
#     batch 2:  sample 3  → compute NE_6  (from all 6 samples)
#     batch 3:  sample 3  → compute NE_9  (from all 9 samples)
#     …
#     stop when |NE_k - NE_{k-batch}| <= ne_thr, or max batches reached.
#
#   This applies to BOTH:
#     - Step 1: question-only NE (rejection feature)
#     - Step 2: with-passage NE (rejection feature, per-passage batches)
#
#   Answer candidate sampling (for C_agg construction) still uses the full M
#   samples per passage, since cluster confidences need stable frequencies.
#
# New CLI flags:
#   --ne_batch_size      Mini-batch size (default 3)
#   --ne_max_batches     Max batches before stopping (default 5, so max 15 samples)
#   --ne_thr             Convergence threshold for |ΔNE| (default 0.05)
...
def refusal_metrics_components(labels_ans: List[int], rejections: List[bool]) -> Dict[str, float]:
    n = len(labels_ans)
    if n == 0:
        return {"p_joint": 0.0, "p_unans": 0.0, "p_reject": 0.0,
                "r_refuse": 0.0, "p_refuse": 0.0, "f1_paper": 0.0}
    p_unans = sum(1 for y in labels_ans if y == 0) / n
    p_reject = sum(1 for r in rejections if r) / n
    p_joint = sum(1 for y, r in zip(labels_ans, rejections) if (y == 0 and r)) / n
    r_refuse = (p_joint / p_unans) if p_unans > 0 else 0.0
    ...
    denom = max(2.0 - p_refuse * r_refuse, 1e-12)
    f1_paper = (p_refuse * r_refuse) / denom
    return {"p_joint": float(p_joint), "p_unans": float(p_unans), "p_reject": float(p_reject),
            "r_refuse": float(r_refuse), "p_refuse": float(p_refuse), "f1_paper": float(f1_paper)}
...
def save_coverage_plot(expected: List[float], empirical: List[float], title: str, path: str) -> None:
    ...
    plt.xlabel("Expected Coverage (1 - alpha)"); plt.ylabel("Empirical Coverage")
...
def cache_key_adaptive_batch(
    question: str, passage: Optional[str], batch_size: int, batch_idx: int, model: str
) -> str:
    """Cache key for adaptive NE sampling.  Each (question, passage, batch_idx) gets
    its own entry so that partial runs can resume."""
    h = hashlib.sha256()
    h.update(b"adaptive||")
    h.update(norm_text(question).encode("utf-8"))
    h.update(b"||")
    if passage is not None:
        ...
        h.update(b"__q_only__")
    h.update(b"||")
    h.update(str(batch_size).encode("utf-8"))
    h.update(b"||")
    h.update(str(batch_idx).encode("utf-8"))
    h.update(b"||")
    h.update(model.encode("utf-8"))
    return h.hexdigest()
...
def llm_sample_answers(
    client: "OpenAI", model: str, question: str, passage: str, M: int,
    temperature: float, max_output_tokens: int, retries: int, retry_sleep_s: float,
) -> List[str]:
    prompt = ZERO_SHOT_TEMPLATE.format(question=question, context=passage)
    return [llm_call_text(client, model, prompt, temperature, max_output_tokens, retries, retry_sleep_s)
            for _ in range(M)]


def llm_sample_answers_question_only(
    client: "OpenAI", model: str, question: str, M: int,
    temperature: float, max_output_tokens: int, retries: int, retry_sleep_s: float,
) -> List[str]:
...
# --------------------------------------------------------------------------
# Adaptive NE sampling
# --------------------------------------------------------------------------
def compute_ne_from_samples(samples: List[str], rouge_thr: float) -> float:
    """Cluster samples and return normalized entropy."""
    clusters = cluster_by_rouge(samples, thr=rouge_thr)
    sizes = [len(c) for c in clusters] if clusters else [1]
    return normalized_entropy_from_cluster_sizes(sizes)


def adaptive_ne_question_only(
    client: "OpenAI",
    model: str,
    question: str,
    ne_batch_size: int,
    ...
    """
    Adaptively sample question-only answers in mini-batches until NE converges.

    Returns: (all_samples, final_NE, num_batches_used)
    """
    all_samples: List[str] = []
    prev_ne: Optional[float] = None

    for batch_idx in range(ne_max_batches):
        # Check cache for this batch
        key = cache_key_adaptive_batch(question, None, ne_batch_size, batch_idx, model)
        if key in cache:
            batch_samples = cache[key]["samples"]
        else:
            batch_samples = llm_sample_answers_question_only(
                client=client, model=model, question=question, M=ne_batch_size,
                temperature=temperature, max_output_tokens=max_output_tokens,
                retries=retries, retry_sleep_s=retry_sleep_s,
            )
            cache[key] = {"samples": batch_samples}

        all_samples.extend(batch_samples)

        # Compute NE over ALL accumulated samples
        current_ne = compute_ne_from_samples(all_samples, rouge_thr)

        # Check convergence (need at least 2 batches)
        if prev_ne is not None and abs(current_ne - prev_ne) <= ne_thr:
            return all_samples, current_ne, batch_idx + 1

        prev_ne = current_ne

    # Exhausted all batches — return final NE
    return all_samples, prev_ne if prev_ne is not None else 0.0, ne_max_batches


def adaptive_ne_with_passage(
    client: "OpenAI",
    model: str,
    question: str,
    passage: str,
    ...
    """
    Adaptively sample passage-conditioned answers in mini-batches until NE converges.
    ...
    for batch_idx in range(ne_max_batches):
        key = cache_key_adaptive_batch(question, passage, ne_batch_size, batch_idx, model)
        if key in cache:
            batch_samples = cache[key]["samples"]
        else:
            batch_samples = llm_sample_answers(
                client=client, model=model, question=question, passage=passage,
                M=ne_batch_size, temperature=temperature,
...
def adaptive_ne_pooled_passages(
    client: "OpenAI",
    model: str,
    question: str,
    passages: List[str],
    ...
    """
    Adaptively sample POOLED across all passages in mini-batches.

    Each batch: sample ne_batch_size from EACH passage, accumulate all,
    compute NE over the pool, check convergence.

    Returns: (per_passage_samples, final_NE, num_batches_used)
    """
    per_passage_samples: List[List[str]] = [[] for _ in passages]
    prev_ne: Optional[float] = None
    ...
    for batch_idx in range(ne_max_batches):
        for passage_idx, passage in enumerate(passages):
            ...
            if key in cache:
                ...
                cache[key] = {"samples": batch_samples}
            per_passage_samples[passage_idx].extend(batch_samples)

        pooled_samples = [s for samples in per_passage_samples for s in samples]
        current_ne = compute_ne_from_samples(pooled_samples, rouge_thr)

        if prev_ne is not None and abs(current_ne - prev_ne) <= ne_thr:
            return per_passage_samples, current_ne, batch_idx + 1

        prev_ne = current_ne

    return per_passage_samples, prev_ne if prev_ne is not None else 0.0, ne_max_batches
...
def run(args: argparse.Namespace) -> None:
    ...
    retriever_set_cap = int(args.max_c_ret_size)

    ne_batch_size = int(args.ne_batch_size)
    ne_max_batches = int(args.ne_max_batches)
    ne_thr = float(args.ne_thr)
    max_ne_samples = ne_batch_size * ne_max_batches

    print(f"\n[Adaptive NE config]")
    print(f"  batch_size={ne_batch_size}  max_batches={ne_max_batches}  "
          f"max_samples={max_ne_samples}  convergence_thr={ne_thr}")
    ...
    if args.plot_coverage:
        ...
        save_coverage_plot(retr_expected, retr_empirical,
    ...
    # ---- Rejection feature sampling with ADAPTIVE NE ----
    rej_feat: List[Dict[str, Any]] = []
    adaptive_passage_samples: Dict[int, List[Tuple[str, List[str]]]] = {}
    early_stops_step1 = 0
    early_stops_step2 = 0
    total_batches_step1: List[int] = []
    total_batches_step2: List[int] = []

    for row_idx, ex in enumerate(tqdm(annotated, desc="Adaptive NE rejection sampling")):
        q = get_question(ex)
        ...
        if not q or not golds:
            ...
            ex["ne_batches_wo"] = None
            ex["ne_batches_with"] = None
            continue

        # ---- Step 1: question-only, adaptive ----
        q_only_samples, NE_wo, batches_wo = adaptive_ne_question_only(
            client=client, model=args.llm_model, question=q,
            ne_batch_size=ne_batch_size, ne_max_batches=ne_max_batches,
            ne_thr=ne_thr, rouge_thr=args.rouge_thr,
            temperature=args.llm_temperature,
            max_output_tokens=args.max_output_tokens,
            retries=args.retries, retry_sleep_s=args.retry_sleep_s,
            cache=cache,
        )
        total_batches_step1.append(batches_wo)
        if batches_wo < ne_max_batches:
            early_stops_step1 += 1

        p_cant_wo = float(NE_wo)
        p_can_wo = float(1.0 - NE_wo)

        # ---- Step 2: with-passage (pooled across C_ret passages), adaptive ----
        ...
        batches_with: Optional[int] = None

        if passages_with:
            per_passage_samples, NE_with, batches_with = adaptive_ne_pooled_passages(
                client=client, model=args.llm_model, question=q,
                passages=passages_with,
                ne_batch_size=ne_batch_size, ne_max_batches=ne_max_batches,
                ne_thr=ne_thr, rouge_thr=args.rouge_thr,
                temperature=args.llm_temperature,
                max_output_tokens=args.max_output_tokens,
            ...
            adaptive_passage_samples[row_idx] = list(zip(passages_with, per_passage_samples))
            with_samples = [s for samples in per_passage_samples for s in samples]
            total_batches_step2.append(batches_with)
            if batches_with < ne_max_batches:
                early_stops_step2 += 1
        ...
        ex["ne_batches_wo"] = batches_wo
        ex["ne_batches_with"] = batches_with
        rej_feat.append({"row_idx": row_idx, "label": label_ans})

    # Print adaptive NE stats
    n_step1 = len(total_batches_step1)
    n_step2 = len(total_batches_step2)
    print(f"\n[Adaptive NE Stats]")
    print(f"  Step 1 (q-only):   {n_step1} questions, "
          f"early-stopped {early_stops_step1}/{n_step1} ({100*early_stops_step1/max(n_step1,1):.1f}%), "
          f"avg batches {np.mean(total_batches_step1):.2f}/{ne_max_batches}")
    if n_step2 > 0:
        print(f"  Step 2 (w/passage): {n_step2} questions, "
              f"early-stopped {early_stops_step2}/{n_step2} ({100*early_stops_step2/max(n_step2,1):.1f}%), "
              f"avg batches {np.mean(total_batches_step2):.2f}/{ne_max_batches}")
    saved_calls_step1 = n_step1 * max_ne_samples - sum(b * ne_batch_size for b in total_batches_step1)
    print(f"  API calls saved (step 1): ~{saved_calls_step1} "
          f"(vs fixed M={max_ne_samples})")

    # ---- Passage-conditioned sampling for answer candidates ----
    # Reuse the adaptive step-2 samples for each row instead of issuing a second
    # fixed-M LLM pass. Each row therefore uses its own early-stopped sample count.
    answer_cands: Dict[int, Dict[str, float]] = {}
    for row_idx, ex in enumerate(tqdm(annotated, desc="Passage-conditioned sampling for answer candidates")):
        ...
        stored_samples = adaptive_passage_samples.get(row_idx, [])
        for _passage, samples in stored_samples:
            if not samples:
                continue
    ...
    if not cal_items:
        raise RuntimeError("No calibration items available for rejection module.")

    cal_ne_wo_all = [float(annotated[it["row_idx"]]["rej_NE_wo"])
                     for it in cal_items if annotated[it["row_idx"]].get("rej_NE_wo") is not None]
    cal_ne_with_all = [float(annotated[it["row_idx"]]["rej_NE_with"])
                       for it in cal_items if annotated[it["row_idx"]].get("rej_NE_with") is not None]
    ...
    def calibrate_step(
        step_name: str, p_can_key: str, p_cant_key: str, ne_key: str,
    ) -> Tuple[Dict[str, float], List[float]]:
        step_items = [it for it in cal_items
                      if annotated[it["row_idx"]].get(p_can_key) is not None
                      and annotated[it["row_idx"]].get(p_cant_key) is not None]
        if not step_items:
        ...
        if len(p_cant_unans) == 0 or len(p_can_ans) == 0:
            raise RuntimeError(f"{step_name} calibration needs both answerable and unanswerable samples.")

        best = {"avg_size": float("inf"), "alpha0": 0.0, "alpha1": 0.0,
                "q_cant": 0.0, "q_can": 0.0, "tau_text": float("inf"), "rcorrect": rcorrect}
        ...
        for alpha0 in alpha0_grid:
            ...
            for it in step_items:
                ridx = int(it["row_idx"])
                ex = annotated[ridx]
                modes[ridx] = decide_rejection_mode(
                    float(ex[p_can_key]), float(ex[p_cant_key]), q_can=q_can, q_cant=q_cant)
            ...
            for it in step_items:
                ...
                mode = decide_rejection_mode(
                    float(ex[p_can_key]), float(ex[p_cant_key]), q_can=q_can, q_cant=q_cant)
                cand_map = answer_cands.get(ridx, {})
                ans_set = [a for a, conf in cand_map.items() if (-float(conf) + 0.01 * NE) <= tau_text]
                if mode == "reject":
                    sizes.append(1)
                elif mode == "add_cant":
            ...
            if avg_size < best["avg_size"]:
                best = {"avg_size": avg_size, "alpha0": float(alpha0), "alpha1": float(alpha1),
                        "q_cant": float(q_cant), "q_can": float(q_can),
                        "tau_text": float(tau_text), "rcorrect": float(rcorrect)}
    ...
    best_step1, best_s_hat_step1 = calibrate_step(
        "Step 1 (only question)", "rej_p_can_wo", "rej_p_cant_wo", "rej_NE_wo")
    best_step2, best_s_hat_step2 = calibrate_step(
        "Step 2 (with passage)", "rej_p_can_with", "rej_p_cant_with", "rej_NE_with")
    ...
    for row_idx, ex in enumerate(tqdm(annotated, desc="Inference: build C_agg with rejection CP")):
        ...
        mode_step1 = decide_rejection_mode(p_can_wo, p_cant_wo,
                                           q_can=q_can_star_step1, q_cant=q_cant_star_step1)
        ...
        if mode_step1 == "reject":
            ...
            if has_step2:
                mode_step2 = decide_rejection_mode(
                    float(ex["rej_p_can_with"]), float(ex["rej_p_cant_with"]),
                    q_can=q_can_star_step2, q_cant=q_cant_star_step2)
    ...
    if args.plot_coverage:
        ...
        for row_idx, ex in enumerate(annotated):
            ...
            mode_step1 = decide_rejection_mode(
                float(ex["rej_p_can_wo"]), float(ex["rej_p_cant_wo"]),
                q_can=q_can_star_step1, q_cant=q_cant_star_step1)
            if mode_step1 == "reject":
                continue
            has_step2 = ex.get("rej_p_can_with") is not None and ex.get("rej_p_cant_with") is not None
        ...
        if best_s_hat_plot and llm_eval_scores:
            ...
            save_coverage_plot(llm_expected, llm_empirical,
    ...
    if args.summary_csv_path:
        ...
        for ex in annotated:
            rows_out.append([
                get_question(ex),
                ex.get("rej_NE_wo"), ex.get("rej_p_can_wo"), ex.get("rej_p_cant_wo"),
                ex.get("rej_NE_with"), ex.get("rej_p_can_with"), ex.get("rej_p_cant_with"),
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser("TRAQ + Adaptive Rejection CP — Adaptive NE Sampling")
    ...
    # sampling (for answer candidates — fixed M)
    p.add_argument("--M", type=int, default=15, help="samples per (q,passage) for answer candidates")

    # Adaptive NE sampling params
    p.add_argument("--ne_batch_size", type=int, default=3,
                    help="Mini-batch size for adaptive NE sampling (default 3)")
    p.add_argument("--ne_max_batches", type=int, default=5,
                    help="Max batches before stopping (default 5, so max 3*5=15 samples)")
    p.add_argument("--ne_thr", type=float, default=0.05,
                    help="Convergence threshold: stop when |NE_k - NE_{k-1}| <= ne_thr")

    # Adaptive Rejection CP
    p.add_argument("--alpha_cp", type=float, default=0.1)
    ...
    # outputs
    p.add_argument("--summary_csv_path", type=str, default="out/adaptive_NE/summary_adaptive.csv")
    p.add_argument("--plot_coverage", action="store_true")
    p.add_argument("--coverage_plot_points", type=int, default=30)
    p.add_argument("--retriever_coverage_plot_path", type=str, default="out/adaptive_NE/retriever_coverage_v4.png")
    p.add_argument("--retriever_coverage_csv_path", type=str, default="out/adaptive_NE/retriever_coverage_v4.csv")
    p.add_argument("--llm_coverage_plot_path", type=str, default="out/adaptive_NE/llm_coverage_plot_v4.png")
    p.add_argument("--llm_coverage_csv_path", type=str, default="out/adaptive_NE/llm_coverage_plot_v4.csv")
    return p
